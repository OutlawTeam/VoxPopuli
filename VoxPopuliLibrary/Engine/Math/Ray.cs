...
namespace VoxPopuliLibrary.Engine.Maths
{
    public class Ray
    {
        ...
        public void TestWithTerrain(Action<Vector3i,Vector3i,string> Callback)
        {
            ...
            List<Tuple<double?, Vector3i,Vector3i ,string>> PossibleCollision = new List<Tuple<double?, Vector3i, Vector3i, string>>();
                        ...
                        if (ClientWorldManager.world.GetChunkManagerClient().GetBlock(i, j, k, out string id))
                        {
                            if (id != "air")
                            {
                                foreach (Collider collider in RessourceManager.RessourceManager.GetPhysicCollider(BlockRegister.BlockList[id].GetCollider()))
                                {
                                    ...
                                    PossibleCollision.Add(new Tuple<double?, Vector3i,Vector3i, string>(entry_time, normal,new Vector3i(i,j,k), id));
                                }
                            }
                        }
                    }
            ...
            {
                (double? entry_time, Vector3i normal,Vector3i position, string id) = PossibleCollision.OrderBy(x => x.Item1).First();
                entry_time -= 0.001f;
